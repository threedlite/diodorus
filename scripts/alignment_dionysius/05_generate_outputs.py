#!/usr/bin/env python3
"""
Generate final outputs for Dionysius of Halicarnassus alignment.

Outputs:
  output/dionysius/alignment_dionysius_perseus.xml  — TEI standoff
  output/dionysius/alignment_dionysius_perseus.tsv  — tabular
  output/dionysius/alignment_report.md           — quality report
"""
...
ALIGNMENTS = PROJECT_ROOT / "build" / "dionysius" / "entity_validated_alignments.json"
OUT_XML = PROJECT_ROOT / "build" / "dionysius" / "alignment_dionysius_perseus.xml"
OUT_TSV = PROJECT_ROOT / "build" / "dionysius" / "alignment_dionysius_perseus.tsv"
OUT_REPORT = PROJECT_ROOT / "build" / "dionysius" / "alignment_report.md"
...
xml_lines = [
    ...
    "    <fileDesc>",
    "      <titleStmt>",
    "        <title>Alignment: Long English (1862) / Perseus Greek Dionysius of Halicarnassus</title>",
    "      </titleStmt>",
    "      <publicationStmt><p>Generated automatically.</p></publicationStmt>",
    "      <sourceDesc>",
    '        <bibl xml:id="long">George Long, Thoughts of Dionysius of Halicarnassus Antoninus (1862). Public domain. Source: Project Gutenberg #15877.</bibl>',
    '        <bibl xml:id="perseus">PerseusDL canonical-greekLit tlg0081.tlg012</bibl>',
    "      </sourceDesc>",
    "    </fileDesc>",
    "  </teiHeader>",
    "  <text>",
...
for book_num in sorted(by_book.keys(), key=int):
    ...
    for a in book_aligns:
        src = f"roberts:book-{book_num}/section-{a.get('english_section', '')}"
        if a.get("greek_cts_ref") is not None:
            tgt = f"urn:cts:greekLit:tlg0081.tlg012.{a['greek_edition']}:{a['greek_cts_ref']}"
...
report = f"""# Dionysius of Halicarnassus Alignment Quality Report

**Date:** {date.today().isoformat()}
**Greek source:** Perseus canonical-greekLit tlg0081.tlg012 (De Compositione Verborum, ed. Usener-Radermacher/Teubner 1904)
**English source:** W. Rhys Roberts (1910), Project Gutenberg #50212
...
- `alignment_dionysius_perseus.xml` — TEI standoff alignment
- `alignment_dionysius_perseus.tsv` — tabular format
- `entity_validated_alignments.json` — full alignment data
- `alignment_quality_map_dionysius.svg` — visual heatmap
"""
