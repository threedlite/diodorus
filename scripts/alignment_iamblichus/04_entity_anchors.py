#!/usr/bin/env python3
"""
Validate Iamblichus alignments with entity anchoring.

Iamblichus is philosophical prose with few proper names, so entity validation
will be weak. We still run it for consistency, using Greek transliteration
matching against any names that do appear.

Input:  output/iamblichus/section_alignments.json
Output: output/iamblichus/entity_validated_alignments.json
"""
...
ALIGNMENTS = PROJECT_ROOT / "build" / "iamblichus" / "section_alignments.json"
OUTPUT = PROJECT_ROOT / "build" / "iamblichus" / "entity_validated_alignments.json"
