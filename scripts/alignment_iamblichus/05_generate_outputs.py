#!/usr/bin/env python3
"""
Generate final outputs for Iamblichus alignment.

Outputs:
  output/iamblichus/alignment_iamblichus_perseus.xml  — TEI standoff
  output/iamblichus/alignment_iamblichus_perseus.tsv  — tabular
  output/iamblichus/alignment_report.md           — quality report
"""
...
ALIGNMENTS = PROJECT_ROOT / "build" / "iamblichus" / "entity_validated_alignments.json"
OUT_XML = PROJECT_ROOT / "build" / "iamblichus" / "alignment_iamblichus_perseus.xml"
OUT_TSV = PROJECT_ROOT / "build" / "iamblichus" / "alignment_iamblichus_perseus.tsv"
OUT_REPORT = PROJECT_ROOT / "build" / "iamblichus" / "alignment_report.md"
...
xml_lines = [
    ...
    "    <fileDesc>",
    "      <titleStmt>",
    "        <title>Alignment: Long English (1862) / Perseus Greek Iamblichus</title>",
    "      </titleStmt>",
    "      <publicationStmt><p>Generated automatically.</p></publicationStmt>",
    "      <sourceDesc>",
    '        <bibl xml:id="long">George Long, Thoughts of Iamblichus Antoninus (1862). Public domain. Source: Project Gutenberg #15877.</bibl>',
    '        <bibl xml:id="perseus">PerseusDL canonical-greekLit tlg2023</bibl>',
    "      </sourceDesc>",
    "    </fileDesc>",
    "  </teiHeader>",
    "  <text>",
...
for book_num in sorted(by_book.keys()):
    ...
    safe_book = book_num.replace(" ", "-")
    xml_lines.append(f'      <linkGrp type="alignment" subtype="{safe_book}">')
    for a in book_aligns:
        src = f"taylor:{safe_book}/section-{a.get('english_section', a.get('english_cts_ref', ''))}"
        if a.get("greek_cts_ref") is not None:
            tgt = f"urn:cts:greekLit:tlg2023.{a['greek_edition']}:{a['greek_cts_ref']}"
...
report = f"""# Iamblichus Alignment Quality Report

**Date:** {date.today().isoformat()}
**Greek source:** Perseus canonical-greekLit tlg2023 (De Vita Pythagorica / De Mysteriis, First1KGreek)
**English source:** Thomas Taylor, Project Gutenberg #63300 / #72815
...
- `alignment_iamblichus_perseus.xml` — TEI standoff alignment
- `alignment_iamblichus_perseus.tsv` — tabular format
- `entity_validated_alignments.json` — full alignment data
- `alignment_quality_map_iamblichus.svg` — visual heatmap
"""
